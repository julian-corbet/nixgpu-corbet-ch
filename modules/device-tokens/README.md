...
```nix
[
  { name = "rocm-compute"; count = 2; paths = [ "/dev/kfd" "/dev/dri/by-vendor/amd-render" "/dev/dri/by-vendor/amd-card" ]; }
  { name = "vcn";          count = 2; paths = [ "/dev/dri/by-vendor/amd-render" "/dev/dri/by-vendor/amd-card" ]; }
]
```

**Device indexes are not portable — use the vendor-keyed paths, not a numbered
`/dev/dri/cardN`.** That numbering depends on enumeration order on the
specific machine (a BMC/IPMI virtual VGA adapter frequently claims `card0`,
pushing the real GPU to `card1` or further), and that order is not guaranteed
stable across a kernel update, a firmware update, or even boot-to-boot under
an async-probe kernel — getting this wrong is silent at apply time. The
defaults above reference `/dev/dri/by-vendor/amd-card`/`-render`, generated by
the sibling `nixgpu.nixosModules.stableDevicePaths` NixOS module (a udev rule
matching by PCI vendor ID instead of enumeration index) — **enable that
module on the GPU-bearing host** alongside this one:
